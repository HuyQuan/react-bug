    ...
    Api.get('/blog/rate/' + params.id)
      .then(response => {
             let sumRate = 0;
               ...
               Object.values(response.data.data).map((object) => {
                   return sumRate += object.rate
               })
             let avgRate = sumRate / Object.keys(response.data.data).length
  ...
  function fetchData() {
    if (item) {
          return (
              ...
              <ListComment comments={ comments } getIdSubComment={ getIdSubComment } idBlog={ params.id } />
              <Comment idBlog={params.id} idSubComment={ idSubComment } addComment={ addComment }/>
              </div>
            );
      }  
  }
